...
type NonEmptyBlockStatement = t.BlockStatement & { body: [t.Statement, ...t.Statement[]] }
...
class TokenPrintSession {
    ...
    *print(file: t.File): Iterable<Token> {
    ...
    private canCollapse(node: t.Statement): node is NonEmptyBlockStatement {
        return (
    ...
    private *body(node: t.Statement): Iterable<Token> {
        if (this.canCollapse(node)) yield* this.node(node.body[0])
    ...
    private *nodeContent(node: t.Node, _ctx?: Ctx): Iterable<Token> {
        switch (node.type) {
            ...
            case 'Identifier':
                ...
                return
            case 'PrivateName':
                yield { type: 'private-ident', value: node.id.name }
                return
            case 'StringLiteral':
                yield { type: 'string-literal', value: JSON.stringify(node.value) }
                return
            ...
            case 'OptionalCallExpression':
                ...
                yield this.fixed('(')
                yield* this.comments(node.innerComments)
                yield* this.expressionList(node.arguments, node)
                yield this.fixed(')')
                return
            case 'NewExpression':
            ...
            case 'OptionalMemberExpression':
                yield* this.expr(node.object, node, 'object')
                if (node.computed) {
                    if (node.optional) yield this.fixed('?.')
                    yield this.fixed('[')
            ...
            case 'ExportSpecifier':
                if (node.exportKind === 'type') yield this.fixed('type')
                yield* this.node(node.local)
                if (node.exported.type !== 'Identifier' || node.exported.name !== node.local.name) {
                    yield this.fixed('as')
