...
The leandir contains no copy of the source configuration. LeanPrint writes flat workspace state and file baselines to `leandir-lock.json`, always reloads settings from the source root, and keeps the lock 2-space formatted. Add `leandir-lock.json` to the source project's `.gitignore` so an accidentally copied lock is not committed. The lock records a canonical SHA-256 hash of the fully resolved source configuration; JSON formatting is irrelevant. If source configuration changes, `pull` asks you to run `push`. An integrity hash detects accidental edits to the lock itself.
